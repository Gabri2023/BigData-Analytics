...
def extract_process_metrics(log_path: Union[str, Path], top_k: int = 3) -> Tuple[List[str], List[str], List[str]]:
    ...
    Domain-independent: handles artificial Start/End nodes automatically.
    """
    ...
    # --- SMART EARLY & LATE EXTRACTION (Domain-Independent) ---
    raw_start = list(start_activities.keys())
    raw_end = list(end_activities.keys())
    
    real_early = set(raw_start)
    real_late = set(raw_end)
    
    # Navigate the DFG to find immediate successors of starts and predecessors of ends
    for (source, target) in dfg.keys():
        if source in raw_start:
            real_early.add(target)
        if target in raw_end:
            real_late.add(source)
            
    # Remove generic/artificial nodes that won't appear in custom subgraphs
    artificial_tokens = ['start', 'end', 'start_event', 'end_event']
    early_labels = [node for node in real_early if str(node).lower() not in artificial_tokens]
    late_labels = [node for node in real_late if str(node).lower() not in artificial_tokens]
